...
module Data.Profunctor.Optic.Type (
  ...
  , Coscanning, Coscanning1
  , Conjoining
  , CoercingL, CoercingR
    -- * Optic
  , Optic, StarLike, CostarLike
  , Optic', StarLike', CostarLike'
    -- * Equality
  , Equality, Equality'
    -- * Iso
...
type Coscanning p = (Cotraversing p, Traversable (Corep p))

type Coscanning1 p = (Cotraversing1 p, Traversable1 (Corep p))

type Conjoining p = (Scanning p, Coscanning p, Adjunction (Corep p) (Rep p))
...
type StarLike f s t a b = Optic (Star f) s t a b

type CostarLike f s t a b = Optic (Costar f) s t a b

type Optic' p s a = Optic p s s a a

type StarLike' f s a = StarLike f s s a a

type CostarLike' f t b = CostarLike f t t b b
...
--
type Cotraversal s t a b = forall p. (Closed p, Cotraversing p) => Optic p s t a b
...
type Scan s t a b = forall p. (Affine p, Scanning p) => Optic p s t a b

type Scan1 s t a b = forall p. (Strong p, Scanning1 p) => Optic p s t a b

-- | \( \quad \mathsf{Coscan}\;S\;A = \exists n : \mathbb{N}, S \cong \mathsf{Fin}\,n \to A \)
--
-- See also section 3 on Kaleidoscopes < https://cs.ttu.ee/events/nwpt2019/abstracts/paper14.pdf here >.
--
type Coscan s t a b = forall p. (Closed p, Coscanning p) => Optic p s t a b

type Coscan1 s t a b = forall p. (Coaffine p, Coscanning1 p) => Optic p s t a b

type Scan' s a = Scan s s a a

type Scan1' s a = Scan1 s s a a

type Coscan' t b = Coscan t t b b

type Coscan1' t b = Coscan1 t t b b

---------------------------------------------------------------------
-- Adjoint
---------------------------------------------------------------------

type Adjoint s t a b = forall p. (Affine p, Coaffine p, Conjoining p) => Optic p s t a b
...
instance Coapplicative f => Choice (Costar f) where
  ...
  right' (Costar f) = Costar $ either (Left . copure) (Right . f) . coapply
#endif
