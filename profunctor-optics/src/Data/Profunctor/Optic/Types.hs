...
type Ixed f = (K.Lookup f, K.Adjustable f)

type MonoIxed a = (MK.MonoLookup a, MK.MonoAdjustable a)
...
type Cxgrate' k t b = Cxgrate k t t b b
...
type Cxtraversal0 k s t a b = forall p. Coaffine p => CoindexedOptic p k s t a b

type Cxtraversal k s t a b = forall p. (Coaffine p, Cotraversing p) => CoindexedOptic p k s t a b

type Cxtraversal1 k s t a b = forall p. (Closed p, Cotraversing1 p) => CoindexedOptic p k s t a b
...
type Cxfold0 k t b = forall p. (Coaffine p, CoercingL p) => CoindexedOptic' p k t b

type Cxfold k t b = forall p. (Affine p, Cotraversing p, CoercingL p) => CoindexedOptic' p k t b

type Cxfold1 k t b = forall p. (Choice p, Cotraversing1 p, CoercingL p) => CoindexedOptic' p k t b
...
type Rxsetter k s t a b = forall p. (Coaffine p, Cotraversing p, Remapping p) => CoindexedOptic p k s t a b

type Rxsetter1 k s t a b = forall p. (Closed p, Cotraversing1 p, Remapping1 p) => CoindexedOptic p k s t a b
...
type Rxview k t b = forall p. (Closed p, CoercingL p) => CoindexedOptic' p k t b
