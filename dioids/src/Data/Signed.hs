...
-- | 'Sign' is isomorphic to '(Any,Any)' as a monoid, but has a distinct poset ordering:
--
-- @ 'Indeterminate' >= 'Positive' >= 'Zero'@ and
-- @ 'Indeterminate' >= 'Negative' >= 'Zero'@ 
--
-- In particular, 'Positive' and 'Negative' are not comparable as they are in '(Any,Any)':
--
...
-- | Safe constructor.
signed :: Monoid a => Sign -> a -> Signed a
signed Zero _ = Signed Zero mempty
signed s    a = Signed s a
...
instance Monoid a => Monoid (Signed a) where
  ...
  mempty = Signed Zero mempty

instance (Monoid a, Semiring a) => Semiring (Signed a) where

  _ >< Signed Zero _ = Signed Zero zero
  Signed Zero _ >< _ = Signed Zero zero
  
  Signed s a >< Signed t b = Signed (s><t) (a><b) 

  fromBoolean = fromBooleanDef (Signed Positive one)
